...
- Para realizar um cálculo de custo, o sistema deverá permitir filtrar um determinado conjunto de
produtos, de modo que apenas aqueles cujo valor unitário for superior à média deverão estar
...
O retorno da função obrigatoriamente deve ser uma lista. Cada elemento da lista é uma
tupla em que a primeira posição contém o nome do produto e a segunda, o respectivo preço.
...
def maiores_que_media(conteudo:dict)->list:
    ...
    for produto, preco in conteudo.items():
        if preco > media:
            acima_da_media.append(tuple((produto, preco)))
